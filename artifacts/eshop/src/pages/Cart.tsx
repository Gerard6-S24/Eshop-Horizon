        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-xl p-5 border border-[#E8DFC8]">
              {freeShipping ? (
                ...
                <p className="text-gray-600 text-sm mb-2">Plus que <strong className="text-[#4A7C59]">{formatPrice(remaining)}</strong> pour la livraison gratuite</p>
              )}
              <div className="w-full bg-[#E8DFC8] rounded-full h-2">
                <div className="bg-[#4A7C59] h-2 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
              </div>
            </div>

            {items.map(item => (
              <div key={item.id} className="bg-white rounded-xl p-5 border border-[#E8DFC8] flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-[#2D2D2D] text-sm leading-tight">{item.name}</h3>
                  <p className="text-[#4A7C59] font-bold mt-1">{(item.price * item.quantity).toFixed(2).replace(".", ",")}€</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex items-center border-2 border-[#E8DFC8] rounded-lg overflow-hidden">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="px-2 py-1.5 text-[#4A7C59] hover:bg-[#F5EDD7]"><Minus size={14} /></button>
                    <span className="px-3 py-1.5 text-sm font-semibold min-w-[2rem] text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="px-2 py-1.5 text-[#4A7C59] hover:bg-[#F5EDD7]"><Plus size={14} /></button>
                  </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl p-6 border border-[#E8DFC8] sticky top-24">
              ...
              <Link href="/checkout" className="btn-primary w-full block text-center no-underline mb-3">
                Passer la commande
              </Link>
